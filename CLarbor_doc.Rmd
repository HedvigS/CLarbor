...
![Pruned Chu-Liu graph of Sahul features (uncontrolled)](output/Vizualisations/CLarbor_plots/Sahul_best_CLarbor_uncontrolled_graph_pruned.pdf)

```{r}
read_tsv("output/CLarbors/Sahul_best_CLarbor_uncontrolled.tsv") %>% 
...
![pruned Chu-Liu graph of Sahul features (controlled)](output/Vizualisations/CLarbor_plots/Sahul_best_CLarbor_controlled_graph_pruned.pdf)

```{r}
read_tsv("output/CLarbors/Sahul_best_CLarbor_controlled.tsv") %>% 
  dplyr::select(Target_feature = `Target feature`, Weight) %>%
  distinct() %>% 
  ggplot() +
  geom_line(aes(x = fct_reorder(Target_feature, Weight), y = Weight, group = 1)) +
