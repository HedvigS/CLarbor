...
Feature meta-data file. This is used for the graphs of the CLarbors. It needs to have the following columns: Feature_ID, Abbreviation and group. Feature_ID needs to correspond to the column names in `value_df`. The abbreviations are going to be used as the names of the nodes i the CLarbor graph and the group is used to color similar features by the same colour. If you don't have any abbreviations for the features that you wish to use, or haven't grouped the features then unfortunately, for now, you just need to bear with us and copy over the Feature_ID and but the same characthers string for every row in the group-column.
...
![Differences between 1 and 2](output/Vizualisations/difference_maps/Sahul_diff1_map_unweighted_vs_weighted_uncontrolled.pdf)
...
![Differences between 2 and 3](output/Vizualisations/difference_maps/Sahul_diff2_map_weighted_uncontrolled_vs_weighted_controlled.pdf)
...
![Neighbour-joining trees](output/Vizualisations/distance_based/Neighbour_joining_trees/Sahul_NJT_all_three.pdf)
...
![The first and second dimensions of the MDS analysis of each of the three datasets, coloured by macroarea](output/Vizualisations/distance_based/MDS/Sahul_MDS_all_three.pdf)
