...
dependency_matrix_cond_MI <- function(value_df, external_variables_df = tibble(rowname = value_df[[1]], external_variables_united = "null")){ 
  ...
  library(tidyverse)
  library(infotheo) #this package contains the functions for computing conditional mutual information and entropy
  ...
  dfs_joined <- cbind(value_df_for_depfun, external_variables_df_for_depfun)
  ...
  depfun <- function(x,y){
    if (x == y){
      ...
      d <- dfs_joined %>% 
        select(Var1 = x, Var2 = y, external_variables_united) %>% 
        drop_na()
  ...
  message(paste("The adjacency matrix for the weighted directed graph, the maximum spanning tree, is now created. There were", sum(is.nan(dependencys)), "cells with missing dependency values. These are the result of too few observations given the pair of features."))
