...
write_tsv(dep_matrix_controlled %>% as.data.frame() %>% rownames_to_column(),
          "output/depmatrices/Sahul_feature_dependencies_controlled.tsv")
...
write_tsv(as_tibble(best_CLarbor_df) %>% type_convert(), "output/CLarbors/Sahul_best_CLarbor_controlled.tsv")
write_file(best_CLarbor$root, "output/CLarbors/Sahul_best_root_controlled.txt")

all_CLarbors <- best_CLarbor$all.arbors
try(save(all_CLarbors, file = "output/CLarbors/Sahul_all_CLarbors_controlled.RData"))
